...
# ind_ct <- nearZeroVar(set14_CT[,21:ncol(set14_CT)], saveMetrics = TRUE)
# ind_jhb <- nearZeroVar(set14_JHB[,21:ncol(set14_JHB)], saveMetrics = TRUE)
# 
# good_ct <- set14_CT[,21:ncol(set14_CT)][,!ind_ct$zeroVar]
# good_jhb <- set14_JHB[,21:ncol(set14_JHB)][,!ind_jhb$nzv]
# 
# catSet14_CT <- data.frame(cbind(set14_CT[,1:20], good_ct))
# catSet14_JHB <- data.frame(cbind(set14_JHB[,1:20], good_jhb))
# 
# saveRDS(catSet14_CT, "catSet14_CT.rds")
# saveRDS(catSet14_JHB, "catSet14_JHB.rds")
# 
# nuSet14_CT <- data.frame(cbind(set14_CT[,1:20], good_ct))
# nuSet14_JHB <- data.frame(cbind(set14_JHB[,1:20], good_jhb))

#setting the ordered variables as scaled numerical:
set14_min$age <- scale(as.numeric(set14_min$age))
set14_min$edu <- scale(as.numeric(set14_min$edu))
set14_min$hh_inc <- scale(as.numeric(set14_min$hh_inc))
set14_min$lsm <- scale(as.numeric(set14_min$lsm))
...
set14_min$sex <- factor(set14_min$sex,
                        levels = c(1,2),
                        labels = c("male", "female"))
set14_min$race <- factor(set14_min$race,
                         levels = c(1,2,3,4),
                         labels = c("black", "coloured", "indian", "white"))
set14_min$lifestages <- factor(set14_min$lifestages,
...
set14_min$mar_status <- factor(set14_min$mar_status,
                               levels = c(1,2,3,4,5),
                               labels = c("single", "married or living together", "widowed", "divorced", "separated"))
set14_min$lifestyle <- factor(set14_min$lifestyle,
...
set14_min$attitudes <- factor(set14_min$attitudes,
...
jpeg("nScree_14_min")
plotnScree(nS, main = "National") # optimal = 4
dev.off()
...
library(psych)
set.seed(123)
fact_14 <- fa(set14_min[7:ncol(set14_min)], nfactors = 6, fm = "pa") # default rotation oblimin, so does allow correlation between factors
...
fact_14_scores_model_08 <- predict(fact_08, data = set14_min[,7:ncol(set14_min)])

# save for use in longitudinal stuff:
saveRDS(fact_14_scores_model_08, "fact_14_scores_model_08.rds")
...
# Investigate(pca_14_nat, "factoInvestigate_14_nat.Rmd", c("pdf_document"))

# contributions plots
jpeg("contributions14_nat_1n2.jpeg")
fviz_pca_var(pca_14_nat,
             axes = c(1,2),
             col.var="contrib",
...
jpeg("contributions14_nat_3n4.jpeg")
fviz_pca_var(pca_14_nat,
             axes = c(3,4),
             col.var="contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
...
jpeg("contributions14_nat_5n6.jpeg")
fviz_pca_var(pca_14_nat,
             axes = c(5,6),
             col.var="contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
...
# for National
vehicle_nat <- rownames(pca_14_nat$var$cor)
corr_nat <- as.data.frame(pca_14_nat$var$cor)
contrib_nat <- as.data.frame(pca_14_nat$var$contrib)
cos2_nat <- as.data.frame(pca_14_nat$var$cos2)
...
dimdesc_14_nat <- dimdesc(pca_14_nat, c(1:npc), proba = 1)

# categorical supplementaries per dimension ... need to explain and interpret "Estimate"
cat_coord_14_nat <- list()
for(i in 1:npc) {
        temp1 <- dimdesc_14_nat[[i]]$category[order(dimdesc_14_nat[[i]]$category[,1], decreasing = TRUE),]
        temp2 <- temp1[c(1:5, (nrow(temp1) - 4): nrow(temp1)),1]
        cat_coord_14_nat[[i]] <- data.frame(Est = round(temp2, 2))
        # write.table(cat_coord_14_nat[[i]], file = paste0("cat_dim", i, "_nat.csv")) # if needed
        
}

# getting tables 
# Dim 1 CT
tab_nat_cat1 <- tableGrob(cat_coord_14_nat[[1]], theme = ttheme_minimal(base_size = 12)) # table
...
# Dim 2 CT
tab_nat_cat2 <- tableGrob(cat_coord_14_nat[[2]], theme = ttheme_minimal(base_size = 12)) # table
...
# Dim 3 CT
tab_nat_cat3 <- tableGrob(cat_coord_14_nat[[3]], theme = ttheme_minimal(base_size = 12)) # table
...
# Dim 4 CT
tab_nat_cat4 <- tableGrob(cat_coord_14_nat[[4]], theme = ttheme_minimal(base_size = 12)) # table
...
# Dim 5 CT
tab_nat_cat5 <- tableGrob(cat_coord_14_nat[[5]], theme = ttheme_minimal(base_size = 12)) # table
...
# Dim 6 CT
tab_nat_cat6 <- tableGrob(cat_coord_14_nat[[6]], theme = ttheme_minimal(base_size = 12)) # table
...
cont_corrs_14_nat <- matrix(0, nrow = 4, ncol = 0)
for(i in 1:npc) {
        temp1 <- as.matrix(dimdesc_14_nat[[i]]$quanti)
        temp2 <- temp1[which(rownames(temp1) %in% c("age", "edu", "hh_inc", "lsm")),]
        cont_corrs_14_nat <- as.data.frame(round(cbind(cont_corrs_14_nat, temp2), 2))
}
names(cont_corrs_14_nat) <- c("Dim1", "pVal","Dim2", "pVal", "Dim3", "pVal", "Dim4", "pVal", "Dim5", "pVal", "Dim6", "pVal" )

# print to file for graphic:

tab_nat_cont <- tableGrob(cont_corrs_14_nat, theme = ttheme_minimal(base_size = 8)) # table
